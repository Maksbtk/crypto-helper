...
if (isset($arResult['ITEM']))
{
    ...
    //делаем ресайзы всех картинок
    $width = 451;
    $height = 678;
    $item['PREVIEW_PICTURE_RESIZE'] = CFile::ResizeImageGet($item['PREVIEW_PICTURE']['ID'], array('width' => $width, 'height' => $height), BX_RESIZE_IMAGE_EXACT, true);
    foreach ($item['JS_OFFERS'] as &$offer) {
        $offer['PREVIEW_PICTURE_RESIZE'] = CFile::ResizeImageGet($offer['PREVIEW_PICTURE']['ID'], array('width' => $width, 'height' => $height), BX_RESIZE_IMAGE_EXACT, true);
    }
    unset($offer);
