	...
	window.JCCatalogItem.prototype = {
			...
			if (intNumber > -1 && intNumber < lineContainer.length)
			{
				...
				if (rowItems && 0 < rowItems.length)
				{
					for (i = 0; i < rowItems.length; i++)
					{
						...
						{
							//custom
							if ($(listContainer).data('type') == 'size') {
								var sizeText =  $(listContainer).find('li[data-onevalue="'+value+'"]').attr('title');
								$(listContainer).siblings('.dropdown-select').find('span').text(sizeText);
							}
							//* custom
		...
		changeInfo: function()
		{
			...
			if (index > -1)
			{
				...
				else
				{
					...
					// show pict and pict_second containers // подмена картинок custom
					if (this.obPict)
					{
						if (this.offers[index].PREVIEW_PICTURE)
						{
							...
							BX.adjust(this.obPict, {attrs: {src: this.offers[index].PREVIEW_PICTURE_RESIZE.src}});
						}
						else if (this.defaultPict.pict.SRC)
						{
						...
						{
							//BX.adjust(this.obPict, {style: {backgroundImage: 'url(\'' + this.defaultPict.pict.SRC + '\')'}});
							BX.adjust(this.obPict, {attrs: {src: '/local/templates/belleyou/img/no_photo.png'}});
						}
		...
		basketResult: function(arResult)
		{
			...
			else
			{
				...
				{
					//$('.button-add-to-cart').text('Нет в наличии');
