...
<link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH?>/css/page-product.css?v=1">

<?php
$basketAction = ['ADD'];

if(empty($_COOKIE['user_city'])){
    $token = "69ef796f7b677f636587c11ced5b1ca8437fcac0";
    ...
    // Получим город или возьмём по умолчанию
    $userCity = !empty($result['location']['data']['city']) ? $result['location']['data']['city'] : "Москва";
    
    setcookie("user_city", $userCity, strtotime( '+30 days' ), "/");
}

$userCity = !empty($_COOKIE['cityName']) ? $_COOKIE['cityName'] : "Москва";
...
<script>
    mindboxViewProduct('<?=$elementId?>');
</script>

<?
$seoRes = [];
$cache = Cache::createInstance();
if ($cache->initCache($arParams['CACHE_TIME'], 'SeoCatalogElement|'.$componentElementParams['ELEMENT_CODE'])) {
    $seoRes = $cache->getVars();
} elseif($cache->startDataCache()) {
    $seoRes = CIBlockElement::GetList([], ['CODE' => $componentElementParams['ELEMENT_CODE']],false, [], ['NAME', 'ID', 'CATALOG_PRICE_1'])->fetch();
    $cache->endDataCache($seoRes);
}

if ($seoRes['NAME'] && $seoRes['CATALOG_PRICE_1']) {

    $price = number_format(floatval($seoRes['CATALOG_PRICE_1']), 0, '.', '') . ' руб.';

    #SALE
    $sale_price = getSaleProductPrice($show_sale, $seoRes['ID'], $seoRes['CATALOG_PRICE_1']);
    if ($show_sale && $sale_price != 0) {
        $price = number_format(floatval($sale_price), 0, '.', '') . ' руб.';
    }
    ##SALE

    $h1 = $seoRes['NAME'];
    $title =  $h1 . ' купить в Москве' . ' по цене ' . $price . ' в интернет-магазине belle you';

    $APPLICATION->SetPageProperty('title', $title);
}

function getSaleProductPrice($show_sale, $prodId, $prodOldPrice) {

    $kbSaleProdArray = Settings::SALE_PRODUCT_ARRAY;
    $sale_price = 0;

    if ($show_sale && $kbSaleProdArray) {
        ...
        if (in_array($prodId,$ids30)) {$sale_price = floor($prodOldPrice - (($prodOldPrice/100)*30));}
        if (in_array($prodId,$ids35)) {$sale_price = floor($prodOldPrice - (($prodOldPrice/100)*35));}
        if (in_array($prodId,$ids40)) {$sale_price = floor($prodOldPrice - (($prodOldPrice/100)*40));}
        if (in_array($prodId,$ids45)) {$sale_price = floor($prodOldPrice - (($prodOldPrice/100)*45));}
        if (in_array($prodId,$ids50) && !in_array($prodId,$ids55)) {$sale_price = floor($prodOldPrice - (($prodOldPrice/100)*50));}
        if (in_array($prodId,$ids55)) {$sale_price = floor($prodOldPrice - (($prodOldPrice/100)*55));}
