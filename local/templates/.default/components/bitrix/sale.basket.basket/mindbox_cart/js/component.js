;(function() {
	...
	BX.Sale.BasketComponent = {
		...
		init: function(parameters)
		{
			...
			//считаем колличесвто товаров в корзине и подставляем в счетчик
			this.setActualCartCount(parameters.result.GRID.ROWS)
		...
		fillTotalBlocks: function()
		{
			...
			if (totalNodes && totalNodes.length)
			{
				...
				//beauty box
				var REMOVE_BEAUTY_BOX = window.siteGetCookie('REMOVE_BEAUTY_BOX') ?? false;
				var BEAUTY_BOX_LIMIT_PRICE = parseInt(this.params.BEAUTY_BOX_LIMIT_PRICE) ?? false;

				if (this.params.BEAUTY_BOX_ENABLED == 'Y' && REMOVE_BEAUTY_BOX == 'Y' && this.result.allSum > BEAUTY_BOX_LIMIT_PRICE)
				{
					$('#beautyBoxBlock').fadeIn(600);

                    $(".addBoxButton").on('click', function(){
						window.siteShowPrelouder();
						window.siteSetCookie('REMOVE_BEAUTY_BOX','', 0);

						$.post( "/ajax/addBox.php", {id: 76243})
                        .done(function() {
                            setTimeout( 'document.location.href="/cart/?reload=y"', 500 );
                        });       
                    });					
                    
                    /*$(document).on('click', '#addBeautyBox', function() {
						window.siteSetCookie('REMOVE_BEAUTY_BOX','', 0);
						window.siteShowPrelouder();
		...
		bindTotalEvents: function(node)
		{
            ...
            //CUSTOM CERT FIELD
            //BX.bind(BX('certificate'), 'change', BX.proxy(this.addCouponAction, this));
            BX.bind(BX('button-cert-apply'), 'click', BX.proxy(this.addCouponActionCustom, this));            
        ...
        //CUSTOM CERT FIELD
        addCouponActionCustom: function(event)
        {
            var target = BX('certificate');
            var certResult = 0;
            
            if (target && target.value)
            {
                $(".cert-error-message").hide();
                $(".balance-message").hide();                
                
                $.post( "/ajax/checkCertApply.php", { COUPON: target.value})
                .done(function( data ) {
                    if(data === 'false'){
                        $(".cert-error-message").text('Сертификат не найден или не активен').show();    
                    }else{
                        target.disabled = true
                        BX.Sale.BasketComponent.actionPool.addCoupon(target.value);                            
                    }    
                });
            }
        },        
		...
		sendRequest: function(action, data)
		{
			...
			BX.ajax({
				...
				onsuccess: BX.delegate(function(result) {
					...
					this.setActualCartCount(result.BASKET_DATA.GRID.ROWS)
		...
		setActualCartCount: function(rows)
		{
			var catrCount = 0;
			$.each(rows, function (index, element) {
				catrCount += element.QUANTITY;
			});
			var coubtEl = $('.basket-items-counter span[data-basket-counter]');
			if (coubtEl.length) {
				coubtEl.text(catrCount);
			}
		},
		...
		createBasketItem: function(itemId)
		{
			...
			if (basketItemTemplate)
			{
				...
				//если по бутибоксу ошибка (количество 0) то просто не отрисовываем его
				/*if (this.params.BEAUTY_BOX_ENABLED == 'Y' && (parseInt(this.items[itemId].PRODUCT_ID) == parseInt(this.params.BEAUTY_BOX_PROD_ID)) && parseInt(this.items[itemId].QUANTITY) <= 0) {
					return;
				}*/
			...
			//скрываем элменты управления количеством и размером для beauty box
			if (this.params.BEAUTY_BOX_ENABLED == 'Y' && (parseInt(this.items[itemId].PRODUCT_ID) == parseInt(this.params.BEAUTY_BOX_PROD_ID)))
			{
				/*if (parseInt(this.items[itemId].QUANTITY) <= 0) {}
				else {}*/
				$('#basket-item-' + itemId + ' .basket-item-options .js-quantity').fadeOut();
			}
		},
		...
		deleteBasketItems: function(items, restore, final)
		{
			...
			var BEAUTY_BOX_ENABLED = this.params.BEAUTY_BOX_ENABLED;
			var BEAUTY_BOX_PROD_ID = this.params.BEAUTY_BOX_PROD_ID;
			var basketItems = this.items;
			$.each(items, function (index, element)
			{
				if (BEAUTY_BOX_ENABLED == 'Y' && (parseInt(basketItems[element].PRODUCT_ID) == parseInt(BEAUTY_BOX_PROD_ID)))
				{
					window.siteSetCookie('REMOVE_BEAUTY_BOX', 'Y', 14);
				}

				//удаляем верстку элемента корзины
				$('#basket-item-' + element).remove();
			});

			//document.location.reload();
            //setTimeout( 'document.location.href="/cart/?reload=y"', 500 );
			setTimeout( 'document.location.href="/cart/"', 500 );
		...
		deleteAction: function()
		{
			...
			ym(24428327,'reachGoal','ecommerce__remove_from_cart',{
				items: [{
					item_id: itemData.ID,
					item_name: itemData.NAME
				}]
				...
				'ecommerce': {
					'remove': {                               // 'remove' actionFieldObject measures.
						'products': [{                          //  removing a product to a shopping cart.
							...
							'variant': itemData.COLOR,
							'quantity': itemData.QUANT
						}]
					}
				}
			});*/
		...
		setAnalyticsDataLayer: function(itemsDiff)
		{
			...
			/*console.log(this.params.DATA_LAYER_NAME)
		...
		getItemAnalyticsInfo: function(itemId, diff)
		{
			...
			return {
				'item_name': this.items[itemId].CONTEXT_PROP.NAME_FOR_SITE || '',
				'item_id': this.items[itemId].REAL_PRODUCT_ID || '',
				'price': this.items[itemId].PRICE || 0,
				//'item_category': '',
				'brand': brand,
				'item_variant': this.items[itemId].CONTEXT_PROP.NAME || '',
				'quantity': Math.abs(diff),
				'currency': this.items[itemId].CURRENCY || ''
			};
		}
	};
})();