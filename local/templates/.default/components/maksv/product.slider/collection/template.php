...
//не работает Asset
Asset::getInstance()->addCss(SITE_TEMPLATE_PATH . "/css/mainpage-section-collection.css?v=1", true);
$userIsAuth = $USER->IsAuthorized();
//echo '<pre>'; var_dump($arResult); echo '</pre>';

?>

<section class="mainpage-section-collection">
    <?if ($arResult['BANNER']['PICTURE'] && $arResult['BANNER']['MOBILE_PICTURE']):?>
        <div class="collection__product-banner">
            <div class="collection__product-banner-inner">
                <picture>
                    <source media="(max-width: 760px)" srcset="<?=$arResult['BANNER']['MOBILE_PICTURE']?>">
                    <img class="collection__product-banner-img" src="<?=$arResult['BANNER']['PICTURE']?>"  alt="invisible">
                </picture>
                <div class="collection__product-banner-content">
                    <div class="h2 collection__product-banner-name"><?=$arResult['BANNER']['NAME']?></div>
                    <?=$arResult['BANNER']['DESCRIPTION_HTML']?>
                    <?/*<p class="collection__product-banner-desc">Линейка бесшовного белья</p>
                    <a class="collection__product-banner-link" href="javascript:void(0)">Смотреть коллекцию</a>*/?>
                </div>
            </div>
        </div>
    <?endif;?>
