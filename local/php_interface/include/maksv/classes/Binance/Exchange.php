...
class Exchange
{
    ...
    public static function screener($interval = '15m', $longOiLimit = 0.99, $shortOiLimit = -0.99, $devMode = false)
    {
        ...
        if ($interval != '15m') {
            sleep(35);
            devlogs('sleep 35' . ' - ' . date("d.m.y H:i:s"), $marketMode . '/screener' . $interval);
        } else {
            sleep(25);
        ...
        foreach ($exchangeBinanceSymbolsList as &$symbol) {
            ...
            try {
                ...
                if (
                    ...
                    && (
                        $actualAdx5m['adx'] < 20
                        || ($actualAdx5m['adx'] < 26 && $actualAdx5m['adxDirection']['isDownDir'] === true)
                    )
                ) {
                ...
                $kline1h = $binanceApiOb->kline($symbolName, '1h', $barsCount, false, false, true, 120);
                ...
                if ($res['screenerPump'] || $res['screenerDump']) {
                    ...
                    if ($screenerData['isLong']){
                        ...
                        $screenerData['mlBoard'] = $mlBoard = 0.72; // 0.72
