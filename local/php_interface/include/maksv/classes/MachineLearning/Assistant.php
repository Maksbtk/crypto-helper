...
class Assistant
{
     ...
     * }
     */
    public static function collectAndStoreTrainData(array $finalResults, string $market, $bybitApiOb, $binanceApiOb, $okxApiOb): array
    {
        $trainDataFile = $_SERVER["DOCUMENT_ROOT"] . '/'.self::$prefix.'/data/train_data.json';
        $allData = [];
        $errors = [];
        ...
        foreach ($finalResults as $res) {
            ...
            if ($market === 'bybit') {
                ...
                try {
                    $bars = $okxApiOb->getCandlesHist($symbolName, '15m', $startTimeMs, $endTimeMs, $useCache, $cacheTime);
                } catch (\Exception $e) {
                    $errors[] = "[{$symbolName}] Ошибка Okx API: " . $e->getMessage();
                    continue;
                }
                if (empty($bars) || !is_array($bars)) {
                    $errors[] = "[{$symbolName}] Нет данных от Okx";
                    continue;
                }
                $rawBars = $bars;
                usort($rawBars, fn($a, $b) => $a[0] <=> $b[0]);
     ...
     * }
     */
    public static function predictRes(array $finalResults, string $market, $bybitApiOb, $binanceApiOb, $okxApiOb): array
    {
        $results = [];
        ...
        foreach ($finalResults as $res) {
            ...
            try {
                $payloadCandles = self::extractNBars($res, $market, $bybitApiOb, $binanceApiOb , $okxApiOb) ?? [];
            } catch (\Exception $e) {
     ...
     */
    private static function extractNBars($res, $market, $bybitApiOb, $binanceApiOb, $okxApiOb, $marketMode): ?array
    {
        $symbolName = $res['symbolName'];
        $rawDate    = $res['date'];
        ...
        if (!$marketMode) {
            ...
            if ($needApiFetch) {
                ...
                if ($market === 'bybit') {
                    ...
                    $limit = self::$nBars;
                    try {
                        $bars = $okxApiOb->getCandlesHist($symbolName, '15m', $startTimeMs, $endTimeMs, true, 86400);
                    } catch (\Exception $e) {
                        $errorsLocal[] = "Ошибка Binance API: " . $e->getMessage();
                        $entry['errors'] = array_merge($entry['errors'], $errorsLocal);
                        $results[$symbolName . '_' . $signalTimestampMs] = $entry;
     ...
     * @return array
     */
    public static function predictResBatch(array $finalResults, string $market, $bybitApiOb, $binanceApiOb, $okxApiOb, $marketMode = false): array
    {
        ...
        foreach ($finalResults as $res) {
            ...
            $candles = self::extractNBars($res, $market, $bybitApiOb, $binanceApiOb, $okxApiOb, $marketMode);
            if ($candles === null) {
        ...
        //echo '<pre>'; var_dump( 'start predictBatch - ' . date("d.m.y H:i:s")); echo '</pre>';
        $responseList = $ml->predictBatch($payloads);
        //echo '<pre>'; var_dump( 'end predictBatch - ' . date("d.m.y H:i:s")); echo '</pre>';
