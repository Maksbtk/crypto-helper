...
(function() {
    ...
    BX.Sale.Checkout = class Checkout {
        ...
        registerOptions(options){
            ...
            this.options.userID = options.userID ?? null
            ...
            this.options.beautybox_id = (typeof options.beautybox_id === 'object' && options.beautybox_id.length) ? options.beautybox_id : []
        ...
        renderFromArgs = (args) => {
            ...
            return Promise.all([
                ...
                new Promise(resolve => {
                    this.renderPaymentMethods(args['PAY_SYSTEM'] ?? [],args['TOTAL'])
                    resolve()
                }),
                new Promise(resolve => {
        ...
        renderDeliveryProps(deliveryProps){
            ...
            jQuery(this.cityField).suggestions({
                ...
                onSelect: BX.proxy(function(suggestion)
                {
                    ...
                    var selectedCity = suggestion.data.city;
                    if  (!selectedCity && suggestion.data.settlement)
                        selectedCity = suggestion.data.settlement;//settlement/settlement_with_type

                    window.siteShowPrelouder();
                    that.getRegionCodeByCityName(selectedCity)
                        .then((code) => {
                            //console.log({code})
        ...
        renderDeliveryAddress(deliveryProps, args){
            ...
            const BoxData = this.analyzeCart()
                ...
                try{
                    ...
                    Object.keys(this.services).forEach(key => {
                        ...
                        tariffList.forEach((tariff, i) => {
                            ...
                            var tariffName = tariff['PROVIDER'];
                            
                            // START TEMP FIX
                            if(minPrice === null || price > minPrice){
                                if(tariff['PROVIDER'] === "dpd" && tariff['NAME'] === "DPD EXPRESS"){
                                    tariffName = "DPD EXPRESS";
                                }else{
                                    return    
                                }
                            }
                            ...
                            //BOX FIX
                            if(BoxData.hasBox && tariffName == "DPD EXPRESS"){
                                // в корзине есть подарочный бокс
                                return
                            }                             
                            //END BOX FIX
                            
                            //BOXBERRY COURIER FIX
                            if(key == "boxberry"){
                                // отключаем курьера boxberry
                                return
                            }                             
                            //END BOXBERRY COURIER FIX                            
                            ...
                            html = tariffName.toUpperCase() + ' (' + html + ')'
                                ...
                                children: [
                                    ...
                                    BX.create({
                                        ...
                                        events: {
                                            click: () => {
                                                if(!input.checked){
                                                    if(tariff['PROVIDER'] === "dpd" && tariffName === "DPD EXPRESS"){
                                                        dpdexpresskey = 1;
                                                    }else{
                                                        dpdexpresskey = 0;    
                                                    }
        ...
        renderPaymentMethods(paymentMethods,totalData){
            const CertificatesData = this.analyzeCart()        
            ...
            paymentMethods.sort(Checkout.PROPS_SORT).forEach(payment => {
                ...
                if(totalData['ORDER_TOTAL_PRICE'] == 0 && payment['ID'] !== '9'){
                    // полная оплата сертификатом
                    return  
                }else if(totalData['ORDER_TOTAL_PRICE'] > 0 && payment['ID'] == '9'){
                    return    
                }
                
                if(CertificatesData.hasBox && payment['ID'] !== '6' && payment['ID'] !== '11'){
                    // в корзине есть подарочный бокс
                    return
                }                
                
                if(CertificatesData.hasCertificates && CertificatesData.hasRegularProducts && payment['ID'] !== '6'){
                    // в корзине и виртуальные и обычные товары
                    return
                }                
        ...
        update(dpdexpress = 0){
            ...
            return new Promise((resolve, reject) => {
                ...
                //First page load dpdexpress check
                if(this.services && dpdexpress == 0){
                    try{
                        Object.keys(this.services).forEach(key => {
                            const service = this.services[key]
                            ...
                            if(!!service['SELECTED']){
                                ...
                                tariffList.forEach(tariff => {
                                    if(!!tariff['SELECTED']){
                                        if(tariff['PROVIDER'] === "DPD EXPRESS" && tariff['NAME'] === "DPD EXPRESS"){
                                            dpdexpress = 1;    
                                        }    
                                    }    
                                })  
                ...
                //console.log("this.options.userID", this.options.userID);
                
                //Global free delivery check on each Update()
                var orderPost = {};
                var currentCountry = this.countries[this.currentCountry] ?? "Россия"
                
                if(currentCountry.toLowerCase() === 'россия'){
                    orderPost['action'] = 'change';
                    orderPost['user'] = this.options.userID;
                    orderPost['country'] = 28023;                            
                    
                    orderPost['dpd'] = dpdexpress;                            
                    
                    BX.ajax.post("/ajax/setOrderData.php", orderPost, (res) => {
                        res = JSON.parse(res)
                    })
                }else{
                    orderPost['action'] = 'change';
                    orderPost['user'] = this.options.userID;
                    orderPost['country'] = 28025;                            
                    orderPost['dpd'] = dpdexpress;                            
                ...
                this.updateTimeout = setTimeout(() => {
                    ...
                    BX.ajax.post(Checkout.AJAX_URL, data, (res) => {
                        ...
                        if(typeof res.order.REDIRECT_URL !== "undefined"){
                            ...
                            if(typeof res.order.ERROR.AUTH[0] !== "undefined"){
                                if(res.order.ERROR.AUTH[0] == "Ошибка регистрации нового пользователя: Указан некорректный номер телефона."){
                                    $('.auth_error').html("Ошибка регистрации нового пользователя:<br>указан некорректный номер телефона.");    
                                }else{
                                    $('.auth_error').text(res.order.ERROR.AUTH[0]);                                    
                                }
        ...
        analyzeCart(){
            ...
            try{
                ...
                const beautybox_id = this.options.beautybox_id
                ...
                if(beautybox_id.length){
                    ...
                    Object.keys(items).forEach(key => {
                        ...
                        const id = product['PRODUCT_ID'] * 1
                        const isBox = beautybox_id.indexOf(id) !== -1
                        if(isBox){
                            info.hasBox = true
                            //console.log('product', id, 'is certificate')
                        }
